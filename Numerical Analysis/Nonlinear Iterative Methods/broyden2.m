...
function[ sol, num_iterations ] = broyden2( x_0, num_iterations )
...
for k = 1:num_iterations
    ...
    % Updating steps
    x_k = sol - H_k * F(sol(1),sol(2));
    s = x_k - sol;
    v = F(x_k(1),x_k(2)) - F(sol(1),sol(2));
    
    H_k = H_k + ((s - H_k*v)*transpose(s)*H_k) / (transpose(s)*H_k*v);
