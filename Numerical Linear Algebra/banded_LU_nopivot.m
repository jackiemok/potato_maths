%% Banded Matrix LU Decomposition without Pivoting
% Perform A = LU decomposition, where A has
% upper bandwidth s_U and lower bandwidth s_L

% Input:    A - (n x n) matrix
%           b - n-length vector
%           xsol - [Optional input] The exact solution used to obtain b
%           s_U - Upper bandwidth s_U
%           s_L - Lower bandwidth s_L
% Output:   L - Lower factor of A
%           U - Upper factor of A
%           x - The computed solution via forward & backward substitutions

function [ L, U, x ] = banded_LU_nopivot( A, b, s_U, s_L, xsol )


[m,n] = size(A);
if m ~= n 
    disp( 'Error: A is not square.' )
    return
end

% Initialize LU factors of A
L = eye(n);
U = A; 

for k = 1:n-1
    for j = k+1:min( n, s_L + k - 1 )
        
        if U(k,k) == 0
            disp( 'Error: A has no unpivoted LU factorization.' )
            return
        end
        
        L(j,k) = U(j,k) / U(k,k);
        
        % Row j <-- Row j - multiplier * (pivot row)
        for m = k:min( s_U + k - 1, n )
            U(j,m) = U(j,m) - L(j,k) * U(k,m);  
        end
    end 
end

%% Solve the system
% Forward substitution for intermediate solution: Ly = b
% (A = LU) => Ax = b <=> (LU)x = b
% <=> L(Ux) = b, where y = (Ux)
y = L \ b;  
% Backward substitution for final solution: Ux = y
x = U \ y; 

%% Analysis
% Compute the residual
r = b - A*x;

rep = sprintf( 'The relative residual norm is: %0.5g', ...
    norm(r) / norm(b) );
disp(rep)

if nargin == 3
    rep = sprintf( 'The relative accuracy is: %0.5g', ...
        norm(xsol - x) / norm(xsol) );
    disp(rep)
end

end